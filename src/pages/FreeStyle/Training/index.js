...
import SubmitBtn from "../../../components/common/SubmitBtn" // batya
...
function Training() {
  ...
  const [isPlaying, setIsPlaying] = useState(0) // batya 
...
function onPlay(){
  ...
  change();
  setIsPlaying(1) // batya
  // audioRef.current.playbackRate = changePercent;
  // <SoundFooter song = {audioSrc}></SoundFooter>
}

function onPause(){
  setIsPlaying(0) // batya
...
const freeStyleFuncs = {
  ...
  return (
    ...
    {isPlaying ?
      <SoundFooter song = {audioSrc}></SoundFooter> : <SubmitBtn name={"Done"} path = {"/team-h/dashBoard"}></SubmitBtn>} 
    </>//batya
  );
}
